...
namespace Gestor_Tecnico
{
    public partial class GestiondeClientes : Form
    {
        ...
        private void CargarClienteEnCampos(DataGridViewRow row)
        {
            ...
            {
                CargarDatosCompletos(txtDNI.Text);
            }
        }

        private void CargarDatosCompletos(string dni)
        {
            try
            {
                using (SqlConnection connection = conexionSQL.ObtenerConexion())
                {
                    ...
                    string query = "SELECT Domicilio, Email FROM Cliente WHERE DNI = @DNI";
            ...
            {
                MessageBox.Show($"Error al cargar datos completos: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        #endregion
    }
